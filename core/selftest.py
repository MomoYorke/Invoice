...
def _test_crediti_in_agenda(r):
    """Le sedute gia' in calendario, per sapere quando serve la prossima fattura.

    I pacchetti si pagano in anticipo. Contare solo le sedute svolte vuol dire
    accorgersi che servono crediti nuovi DOPO l'ultima seduta: tardi. Il
    calendario gli appuntamenti futuri li ha gia', e la sincronizzazione li
    leggeva e li buttava via. Qui si contano, con le stesse regole con cui un
    giorno diventeranno sedute: stesso cliente riconosciuto, stessa regola
    della coppia, stesse esclusioni. Se le regole fossero due, la colonna
    direbbe una cosa e il pacchetto, un mese dopo, un'altra.

    Chiesto il 10.09.2026 guardando Sportsnow, che mette accanto crediti
    rimasti e prenotazioni. La scadenza del pacchetto invece no: nei dati di
    quest'app non esiste, e inventarla sarebbe peggio che non averla.
    """
    import datetime
    import importlib

    def Y():
        return importlib.import_module('sync_sessions')
    from . import sessions as S
    from .db import DEFAULT_SETTINGS
    ...
    try:
        S.configura([
            {'chiave': 'giulia', 'nome': 'Giulia', 'crediti': 10, 'prefisso': 'GIU',
             'prezzi': '', 'fattura_a': '', 'compagno': '', 'attivo': 1},
            {'chiave': 'marco', 'nome': 'Marco', 'crediti': 10, 'prefisso': 'MAR',
             'prezzi': '', 'fattura_a': 'Giulia', 'compagno': 'giulia', 'attivo': 1},
            {'chiave': 'luca', 'nome': 'Luca', 'crediti': 12, 'prefisso': 'LUC',
             'prezzi': '', 'fattura_a': '', 'compagno': '', 'attivo': 0},
            {'chiave': 'nina', 'nome': 'Nina', 'crediti': 5, 'prefisso': 'NIN',
             'prezzi': '', 'fattura_a': '', 'compagno': '', 'attivo': 1},
        ])
        oggi = datetime.date(2026, 9, 10)

        def ev(n, data, titolo, stato='confirmed'):
            return {'id': 'e%d' % n, 'titolo': titolo, 'data': data, 'stato_google': stato}
        eventi = [
            ev(1, '2026-09-09', 'Giulia'),                  # ieri: e' storia, non agenda
            ev(2, '2026-09-10', 'Giulia'),                  # oggi: conta come svolta
            ev(3, '2026-09-12', 'Giulia'),
            ev(4, '2026-09-19', 'Giulia cancelled'),        # consumera' comunque il credito
            ev(5, '2026-09-26', 'Giulia', 'cancelled'),     # eliminata su Google: non c'e' piu'
            ev(6, '2026-09-15', 'Marco'),                   # da solo: credito di Giulia
            ev(7, '2026-09-22', 'Marco'),                   # in coppia: uno a testa
            ev(8, '2026-09-22', 'Giulia'),
            ev(9, '2026-09-13', 'Giulia birthday'),         # non e' una seduta
            ev(10, '2026-09-14', 'Luca'),                   # ex cliente senza pacchetto aperto
            ev(3, '2026-09-12', 'Giulia'),                  # lo stesso evento, letto due volte
            ev(11, '2026-09-11', 'Nina'),
            ev(12, '2026-09-18', 'Nina'),
        ]
        vuoto = {'pacchetti': [], 'esclusi': []}
        _check(r, 'Crediti in agenda', 'si contano solo le sedute dopo oggi, con le regole della sincronizzazione',
               _senza_scoppiare(lambda: Y().in_agenda(vuoto, eventi, oggi)),
               {'giulia': ['2026-09-12', '2026-09-15', '2026-09-19', '2026-09-22'],
                'marco': ['2026-09-22'], 'nina': ['2026-09-11', '2026-09-18']})

        # --- sulla vista: rimasti, in agenda, e la prima seduta scoperta ------
        def pacchetto(pid, cliente, crediti, usati):
            return {'id': pid, 'cliente': cliente, 'crediti': crediti, 'inizio': '2026-08-01',
                    'fine': None, 'fatturato': 'no', 'usati': usati, 'rimasti': crediti - usati,
                    'nota': '', 'sessioni': [{'n': i + 1, 'data': '2026-08-%02d' % (i + 1),
                                              'titolo': cliente, 'cancellata': False}
                                             for i in range(usati)]}
        reg = {'pacchetti': [pacchetto('GIU-01', 'Giulia', 10, 7),     # 3 rimasti
                             pacchetto('NIN-01', 'Nina', 5, 3)],       # 2 rimasti
               'esclusi': []}                                          # Marco: nessuno
        agenda = {'giulia': ['2026-09-12', '2026-09-15', '2026-09-19', '2026-09-22'],
                  'nina': ['2026-09-11'], 'marco': ['2026-09-22']}
        righe = _senza_scoppiare(lambda: S.vista_crediti(reg, agenda))
        _check(r, 'Crediti in agenda', 'accanto ai rimasti ci sono quelle in agenda, e la prima oltre i crediti',
               _senza_scoppiare(lambda: {x['cliente']: (x['rimasti'], x['in_agenda'], x['prima_scoperta'],
                                                        x['tutti_in_agenda']) for x in righe}),
               {'Giulia': (3, 4, '2026-09-22', True), 'Nina': (2, 1, None, False),
                'Marco': (0, 1, '2026-09-22', False)})
        _check(r, 'Crediti in agenda', 'chi ha i crediti gia\' tutti in agenda viene subito dopo i terminati',
               _senza_scoppiare(lambda: [x['cliente'] for x in righe]), ['Marco', 'Giulia', 'Nina'])
        senza = _senza_scoppiare(lambda: S.vista_crediti(reg))
        _check(r, 'Crediti in agenda', 'senza calendario la pagina resta com\'era: nessun numero inventato',
               _senza_scoppiare(lambda: ([x['cliente'] for x in senza],
                                         {x['in_agenda'] for x in senza})),
               (['Marco', 'Nina', 'Giulia'], {None}))

        # Fra chi ha i crediti gia' prenotati, prima la fattura piu' vicina. Sui
        # dati veri del 10.09.2026 l'ordine alfabetico metteva sopra chi aveva
        # ancora dodici sedute coperte e sotto chi andava fatturato entro sette
        # giorni: qui il nome e la data mettono i due clienti in ordine opposto.
        S.configura([
            {'chiave': 'giulia', 'nome': 'Giulia', 'crediti': 10, 'prefisso': 'GIU',
             'prezzi': '', 'fattura_a': '', 'compagno': '', 'attivo': 1},
            {'chiave': 'pierre', 'nome': 'Pierre', 'crediti': 5, 'prefisso': 'PIE',
             'prezzi': '', 'fattura_a': '', 'compagno': '', 'attivo': 1},
        ])
        due = _senza_scoppiare(lambda: S.vista_crediti(
            {'pacchetti': [pacchetto('GIU-01', 'Giulia', 10, 7),
                           pacchetto('PIE-01', 'Pierre', 5, 3)], 'esclusi': []},
            {'giulia': ['2026-09-12', '2026-09-15', '2026-09-19', '2026-09-22'],
             'pierre': ['2026-09-11', '2026-09-12', '2026-09-13']}))
        _check(r, 'Crediti in agenda', 'fra chi ha i crediti gia\' prenotati viene prima la fattura piu\' vicina',
               _senza_scoppiare(lambda: [(x['cliente'], x['prima_scoperta']) for x in due]),
               [('Pierre', '2026-09-13'), ('Giulia', '2026-09-22')])

        # --- quello che si salva vale per il giorno in cui si e' letto ---------
        salvata = _senza_scoppiare(lambda: Y().agenda_da_salvare({'giulia': ['2026-09-12']}, oggi))
        _check(r, 'Crediti in agenda', 'letta oggi, si rilegge uguale',
               _senza_scoppiare(lambda: Y().agenda_salvata(salvata, oggi)), {'giulia': ['2026-09-12']})
        _check(r, 'Crediti in agenda', 'letta ieri non vale: quelle di ieri oggi sono sedute svolte',
               _senza_scoppiare(lambda: Y().agenda_salvata(salvata, oggi + datetime.timedelta(days=1))),
               None)
        _check(r, 'Crediti in agenda', 'niente di salvato, o illeggibile, non e\' zero: e\' «non si sa»',
               _senza_scoppiare(lambda: (Y().agenda_salvata('', oggi), Y().agenda_salvata('{rotto', oggi))),
               (None, None))
    finally:
        S._CONFIG = prima

    # --- e l'app passa davvero di qui ------------------------------------------
    base = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    with io.open(os.path.join(base, 'app.py'), encoding='utf-8') as f:
        sorgente_app = f.read()
    with io.open(os.path.join(base, 'templates', 'credits.html'), encoding='utf-8') as f:
        pagina = f.read()
    _check(r, 'Crediti in agenda', 'la sincronizzazione legge anche l\'agenda e la salva per oggi',
           ('.in_agenda(' in sorgente_app, "'calendario_in_agenda'" in sorgente_app,
            'calendario_in_agenda' in DEFAULT_SETTINGS),
           (True, True, True))
    _check(r, 'Crediti in agenda', 'e la pagina Crediti la mostra',
           ('agenda_salvata(' in sorgente_app, 'r.in_agenda' in pagina, 'r.prima_scoperta' in pagina),
           (True, True, True))
